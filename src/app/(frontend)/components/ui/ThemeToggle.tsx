...
export const ThemeToggle: React.FC = () => {
  ...
  const playDegaussSound = () => {
    try {
      ...
      osc.frequency.exponentialRampToValueAtTime(0.01, audioCtx.currentTime + 0.02);
    ...
    <div className="flex items-center gap-3">
      <div
        ...
        className={`flex p-1 gap-1 border-2 border-terminal-primary bg-terminal-black/80 transition-all group ${
          isDegaussing ? "cursor-wait opacity-80" : "cursor-pointer shadow-[0_0_10px_rgba(var(--terminal-primary-rgb),0.2)] hover:shadow-[0_0_15px_rgba(var(--terminal-primary-rgb),0.4)]"
        }`}
      >
        {/* GREEN SWITCH */}
        <div
      ...
      <div className="hidden sm:block w-[100px]">
        <span className={`text-[10px] font-mono uppercase tracking-[0.2em] block leading-none mb-1 whitespace-nowrap ${isDegaussing ? "text-terminal-primary animate-pulse" : "text-terminal-primary/40"}`}>
